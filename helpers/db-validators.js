...
//Chequea si existe dni en db
const existeDni = async (dni) => {
  const existeDni = await User.findOne({ dni }).exec();
  if (existeDni) {
    throw new Error(`El dni ${dni} ya existe en base de datos`);
  }
};
...
module.exports = { rolValido, existeEmail, existeId, existeDni };
